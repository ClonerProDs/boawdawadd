...
⚡️ Энергия:
    - 20000 энергии | 5 B-Coin 
     🔝 Покупка: Купить флягу 1
    - 60000 энергии | 15 B-Coin
...
async def buy_energi(message):
    ...
    # номер энергии: (кол-во, стоимость)
    energi_list = {
        1: (20000, 5),
        2: (60000, 15)
    }

    data = energi_list.get(u, 'no')
    ...
    await buy_energi_db(user_id, data[0], data[1])
    await message.answer(f'{url}, вы успешно купили флягу и получили {data[0]} энергии за {data[1]} B-Coins {rwin}.')



def reg(dp: Dispatcher):
    dp.register_message_handler(buy_energi, lambda message: message.text.lower().startswith('купить флягу'))
